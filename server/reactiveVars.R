...
clearReactiveVars <- function(allReactiveVars) {
    ...
    allReactiveVars$maPlots$maPlot$maData <- NULL
    allReactiveVars$maPlots$maPlot$maColours <- list(
		Up <- "#B40000",
		Down <- "#00B400",
		Neutral <- "#6B6B6B"
	)
    allReactiveVars$maPlots$maPlot$maZoom <- list(
		x <- NULL,
		y <- NULL
	)
    allReactiveVars$maPlots$maPlot$rendered <- TRUE
